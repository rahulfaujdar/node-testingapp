...
const handler: RequestHandler = async function (req, res){
    const {id, name, email, phone, password, two_factor_authentication} = req.body;

    const query = User.findOne({_id: id});

    const user = await query.exec();

    Object.assign(user, <Partial<UserType>>{
        name,
        email,
        phone,
...
updateRouter.post("/user", authMiddleware(), validate([
    body("id").notEmpty().withMessage("id is required"),
    body("name").notEmpty().withMessage("Name is required")
        .isLength({max: 20}).withMessage("Name must be 20 characters long"),
    body("phone")
        .custom(async (phone, {req}) => {
            const {two_factor_authentication, id} = req.body;
